...
async def on_voice_state_update(m, b, a):
    ...
    c = get_config(m.guild.id); s = Session(); now = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=9)))
    try:
...
async def list_orgs(ctx):
    all_o = fetch_all_orgs(); m = "📋 **団体一覧**\n" + "\n".join([f"・{o.org_name} ({o.alias or 'なし'})" for o in all_o]); await ctx.send(m if all_o else "登録なし")
@bot.command()
@commands.has_permissions(administrator=True)
...
class MultiFunctionView(discord.ui.View):
    ...
    async def sync_all(self, interaction, button):
        ...
        await interaction.response.send_message("現在同期中...", ephemeral=True)
